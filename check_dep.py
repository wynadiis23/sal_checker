...
def find_files():
	...
	if env == 'prod':
		rootDir = read_config_file('user_info', 'dir_dep')	
	...
	folder_tidak_ada = []
	toko_count = 0
	with open("daftartokodep.txt", "r") as a_file:
		...
		#check time. 
		#1. jika waktu skrg kurang dari xjam 3 menit, maka expected mod time adalah x-1jam 33ish menit.
		if current_minute < 10:
			now = datetime.now()
			expected_mod_time = now.replace(hour=now.hour-1, minute=33, second=0).strftime("%H:%M:%S")
			print("modified date yang diekspektasikan %s" %expected_mod_time)
		#2. jika waktu skrg lebih dari xjam 3 menit dan kurang dari xjam 33 menit, maka expected mod time adalah xjam 3ish menit.
		elif current_minute > 10 and current_minute < 40:
			now = datetime.now()
			expected_mod_time = now.replace(hour=now.hour, minute=3, second=0).strftime("%H:%M:%S")
			print("modified date yang diekspektasikan %s" %expected_mod_time)
		#3. jika waktu skrg lebih dari x jam 33 menit, maka expected mod time adalah xjam 33ish menit.
		else:
			now = datetime.now()
			expected_mod_time = now.replace(hour=now.hour, minute=33, second=0).strftime("%H:%M:%S")
			print("modified date yang diekspektasikan %s" %expected_mod_time)
