...
def find_files(environment):
	...
	with open("daftartokodep.txt", "r") as a_file:
		...
		if current_minute > 30:
			now = datetime.now()
			expected_mod_time = now.replace(hour=now.hour, minute=30, second=0).strftime("%H:%M:%S")
		...
		elif current_minute > 0 and current_minute < 30:
			now = datetime.now()
			expected_mod_time = now.replace(hour=now.hour, minute=0, second=0).strftime("%H:%M:%S")
			print("modified date yang diekspektasikan %s" %expected_mod_time)
